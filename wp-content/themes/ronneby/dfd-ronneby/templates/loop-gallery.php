...
if(isset($options['dfd_gallery_layout_style']) && $options['dfd_gallery_layout_style'] == 'standard') {
	$thumb_size = Dfd_Theme_Helpers::postThumbSize(1, $folio_sidebars);
} else {
	$thumb_size = Dfd_Theme_Helpers::postThumbSize($options['dfd_gallery_columns'], $folio_sidebars);
}
...
<div class="dfd-gallery-wrap">
	<div id="dfd-gallery-loop" class="dfd-gallery dfd-gallery-<?php echo esc_attr($options['dfd_gallery_layout_style'] .' '.$page_class) ?>" <?php echo wp_kses_data($data_atts) ?>>
		<?php
		...
		while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			if (has_post_thumbnail()) {
				...
				if(strcmp($options['dfd_gallery_layout_style'], 'fitRows') === 0) {
					...
					$height = $width / $ratio;
					$img_url = dfd_aq_resize($img_url[0], $width, false, true, true, true);
				}
				if(!$img_url) {
					$img_url = $img_src;
				}
					...
					<div class="cover dfd-img-lazy-load <?php echo esc_attr($cover_class) ?>" <?php echo wp_kses_data($animation_data) ?>>
						<div class="dfd-gallery-inner-wrap">
							...
							<div class="entry-thumb <?php echo esc_attr($options['dfd_gallery_comments_likes_style']) ?>">
								<img src="<?php echo esc_attr($dfd_loading_img_src); ?>" alt="<?php echo esc_attr($title); ?>" width="<?php echo esc_attr($img_src[1]) ?>" height="<?php echo esc_attr($img_src[2]) ?>" data-src="<?php echo esc_url($img_url); ?>"/>
