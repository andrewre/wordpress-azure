...
if(!empty($fields)) {
	...
	foreach ($fields as $field_name => $field) {
		...
		if(isset($field['order_no']) && $field['order_no'] !== '') {
			$acf_html[$field['order_no']] = $custom_fields;
		} else if(isset($field['menu_order']) && $field['menu_order'] !== '') {
			$acf_html[$field['menu_order']] = $custom_fields;
		}
	}
	ksort($acf_html);
}
...
<div class="folio-info-field folio-info-field-inner eq-height columns <?php echo esc_attr($desc_columns); ?>">
	<?php if(isset($dfd_ronneby['portfolio_inner_description_title']) && !empty($dfd_ronneby['portfolio_inner_description_title'])) : ?>
		<div class="folio-field-name box-name"><?php echo esc_html($dfd_ronneby['portfolio_inner_description_title']); ?></div>
	<?php endif; ?>
	<?php 
		while (have_posts()) {
			the_post();
...
<div class="folio-info-field folio-add-info columns <?php echo esc_attr($add_info_columns); ?>">
	<div class="box-name"><?php esc_html_e('Info', 'dfd') ?></div>
