...
$thumb_size = Dfd_Theme_Helpers::postThumbSize($options['folio_columns'], $folio_sidebars);
...
<div class="dfd-portfolio-wrap">
	<?php
	echo !empty($sort_panel_html) ? $sort_panel_html : '';
	?>
	<div id="dfd-portfolio-loop" class="dfd-portfolio dfd-portfolio-<?php echo esc_attr($options['folio_layout_style']) ?> <?php echo esc_attr($page_class) ?>" <?php echo wp_kses_data($data_atts) ?>>
		<?php
		...
		while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			?>
				<div class="project <?php echo esc_attr($folio_hover_style_class); ?>" <?php echo wp_kses_data($article_tags_classes) ?>>
					<div class="cover dfd-img-lazy-load <?php echo esc_attr($cover_class) ?>" <?php echo wp_kses_data($animation_data); ?>>
						<div class="entry-thumb <?php echo esc_attr($options['folio_comments_likes_style']) ?>">
							<img src="<?php echo esc_attr($dfd_loading_img_src); ?>" alt="<?php echo esc_attr(get_the_title()); ?>" width="<?php echo esc_attr($img_src[1]) ?>" height="<?php echo esc_attr($img_src[2]) ?>" data-src="<?php echo esc_url($img_url); ?>"/>
						...
						<?php if($options['folio_show_read_more_share'] == 'on') : ?>
							<div class="dfd-read-share clearfix">
								<div class="read-more-wrap">
									<a href="<?php the_permalink(); ?>" class="more-button <?php echo esc_attr($options['folio_read_more_style']) ?>" title="<?php __('Read more','dfd') ?>" data-lang="en"><?php esc_html_e('More', 'dfd'); ?></a>
								</div>
	...
	<?php if ($wp_query->max_num_pages > 1) : ?>
		...
		<nav class="page-nav">
			...
			<?php new Dfd_Ronneby_Pagination(); ?>
